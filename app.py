...
# ----------------------------------------------------------------------------
# File gambar berada satu folder dengan app.py di root repo GitHub
# (lihat struktur repo AdipandangUB/mikroalga_guna-lahan).
# Kode di bawah otomatis mencoba beberapa lokasi umum, jadi tetap aman
# jika suatu saat gambar dipindah ke folder "assets/".
_APP_DIR = Path(__file__).parent
_HEADER_IMG_CANDIDATES = [
    _APP_DIR / "embung-nglanggeran.jpg",
    _APP_DIR / "assets" / "embung-nglanggeran.jpg",
]
HEADER_IMG_PATH = next(
    (p for p in _HEADER_IMG_CANDIDATES if p.exists()), _HEADER_IMG_CANDIDATES[0]
)
...
def render_header_banner(title: str, subtitle: str, img_path: Path):
    ...
    if not img_b64:
        st.warning(
            f"⚠️ Gambar header tidak ditemukan. Sudah dicoba di lokasi berikut:\n\n"
            + "\n".join(f"- `{p}`" for p in _HEADER_IMG_CANDIDATES)
            + "\n\nPastikan file `embung-nglanggeran.jpg` sudah ter-upload ke "
            "repo GitHub (root folder, sejajar dengan `app.py`) lalu deploy ulang."
        )
